...
export const handleChangeSubscription=async (payload:Stripe.Subscription)=>{
    ...
    const status=payload.status=="active"?SubscriptionStatus.ACTIVE:payload.status==="trialing"?SubscriptionStatus.ACTIVE:payload.status==="canceled"?SubscriptionStatus.CANCELED:SubscriptionStatus.EXPIRED;
    ...
    if(!isSubscriptionExist){
        console.log(`Webhook:No Subscription found :${stripeSubscriptionId}`);
        return;
    }
